...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    delivered_grouped = delivered.groupby('order_id').agg({'freight_value': 'sum', 'product_weight_g': 'sum'})

    aggregations = delivered_grouped
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    filtered_dates = orders[pd.to_datetime(orders["order_purchase_timestamp"]).dt.year == 2017].copy()
    ...
    order_purchase_ammount_per_date = filtered_dates.groupby('order_purchase_timestamp').count()
    ...
    # Joining `order_purchase_ammount_per_date` with `filtered_dates` to get the date column
    result_df = pd.merge(order_purchase_ammount_per_date, filtered_dates, on='order_purchase_timestamp')

    # Merging with `holidays` to add the holiday column
    result_df = pd.merge(result_df, holidays, on='date', how='left')

    # Renaming the 'order_purchase_timestamp' column to 'order_count'
    result_df = result_df.rename(columns={'order_purchase_timestamp': 'order_count'})

    # Selecting the 'date' and 'holiday' columns
