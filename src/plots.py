...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    sns.scatterplot(x="weight", y="freight_value", data=df)
    plt.xlabel("Weight")
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    # Get holidays from dataframe
    holidays = df[df['is_holiday'] == True]['date']
    
    # Create a line plot for order amount
    plt.plot(date, order_amount)
    
    # Add vertical lines for holidays
    for holiday in holidays:
        plt.axvline(x=holiday, color='r', linestyle='--')
